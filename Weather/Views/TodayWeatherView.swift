...
struct TodayWeatherView: View {
    ...
    var body: some View {
        VStack(spacing: 10) {
            ...
            HStack(spacing: 20) {
                LottieView(name:
                            cityVM.getLottieAnimation(icon: cityVM.weatherIcon))
                    .frame(width: 100, height: 100)
                
                VStack(alignment: .leading) {
                    Text("\(cityVM.temperature)°F")
     //Shift+Option+8 for the degree symbol
                        .font(.system(size: 42))
            ...
            HStack {
                ...
                widgetView(image: "wind", color: .green, title: "\(cityVM.windSpeed)mi/hr")
                    Spacer()
                    widgetView(image: "wind", color: .green, title: "\(cityVM.humidity)mi/hr")
                    Spacer()
                    widgetView(image: "wind", color: .green, title: "\(cityVM.rainChances)mi/hr")
                    Spacer()
